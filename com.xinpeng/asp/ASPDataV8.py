    ...
    if not header_result:
        ...
        for idx_row in range(len(df_fcst[header_std[0]])):
            if to_split_fcstact(idx_row, file_name[file_idx]):
                ...
                elif not is_ssm(idx_row):
                    add_error_log_and_data(file_name[file_idx], row_data_err, data_err, 'SMM Exception', idx_row)
                elif not validate_kpi(idx_row):
                    add_error_log_and_data(file_name[file_idx], row_data_err, data_err, 'KPI Exception', idx_row)
                ...
                # 判断idx_row的Working/NonWorking列与某几列的关联关系是否符合标准
                    ...
                    add_error_log_and_data(file_name[file_idx], row_data_err, data_err_fcst, 'SMM Exception', idx_row)
                elif not validate_kpi(idx_row):
                    add_error_log_and_data(file_name[file_idx], row_data_err, data_err_fcst, 'KPI Exception', idx_row)
                # 符合所有标准的数据格式化后写入Summary文件
                else:
                    for idx_col in range(len(header_std)):
                        to_replace(idx_col, idx_row)
