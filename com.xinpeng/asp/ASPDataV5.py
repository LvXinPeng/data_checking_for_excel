...
def calc_total_budget(row_idx):
    ...
    is_eq = total_budget == float(df_fcst[header_std[32]][row_idx]) or abs(total_budget - float(df_fcst[header_std[32]][row_idx])) <= 1
    # print(temp)
    # print(total_budget)
...
def validate(row_idx):
    validation = True
    if str(df_fcst[header_std[6]][row_idx]).upper() == 'NONWORKING' and str(df_fcst[header_std[7]][row_idx]) not in ["0", "nan", "N.A."]:
...
def to_simplify(row_idx):
        df_fcst[header_std[10]][row_idx] = str(df_fcst[header_std[10]][row_idx]).replace(str(df_fcst[header_std[3]][row_idx]),'').lstrip()
...
# 创建存放excel的文件夹
file_path_a = "D:/ASP - Erin/Data&Log/"
file_path_b = "D:/ASP - Erin/Error/"
file_path_c = "D:/ASP - Erin/Error/Files"
mkdir(file_path_a)
mkdir(file_path_b)
mkdir(file_path_c)
# 几个生成的excel路径及文件名
