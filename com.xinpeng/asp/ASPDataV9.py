...
def is_duplicate(f_name, row_idx):
    # eg: _issue.xlsx
    issue_name = f_name.split('.xlsx')[0].replace(path, '')[0:8]
        ...
        for idx_row in range(len(df_fcst[header_std[0]])):
            if to_split_fcstact(idx_row, file_name[file_idx]):
                ...
                if not calc_total_budget(idx_row):
                    add_error_log_and_data(file_name[file_idx], row_data_err_calc, data_err, 'TotalBudget Exception',idx_row)
                # 判断idx_row的Issue列是否含有多余值（Issue列值除【文件名内包含的issue值和Act】，其余都视为多余值）
                if not is_duplicate(file_name[file_idx], idx_row):
                    add_error_log_and_data(file_name[file_idx], row_data_err_dupl, data_err, 'Issue Exception', idx_row)
                # 判断idx_row的Working/NonWorking列与某几列的关联关系是否符合标准
                if not validate(idx_row):
                    add_error_log_and_data(file_name[file_idx], row_data_err_vali, data_err, 'Working/NonWorking Exception',idx_row)
               ...
               # 符合所有标准的数据格式化后写入Summary文件
                if is_null(idx_row) and calc_total_budget(idx_row) and is_duplicate(file_name[file_idx], idx_row) and validate(idx_row) and is_ssm(idx_row) and validate_kpi(idx_row):
                    df_fcst[header_std[0]][idx_row] = 'Actual'
                ...
                # 判断idx_row的前两列是否为空
                if not is_null(idx_row):
                    add_error_log_and_data(file_name[file_idx], row_data_err_null_fcst, data_err_fcst,'Issue/Dept Null Exception',idx_row)
                # 判断idx_row的12个月的budget计算总和是否和Total Budget列相等
                if not calc_total_budget(idx_row):
                    add_error_log_and_data(file_name[file_idx], row_data_err_calc_fcst, data_err_fcst, 'TotalBudget Exception',idx_row)
                # 判断idx_row的Issue列是否含有多余值（Issue列值除【文件名内包含的issue值和Act】，其余都视为多余值）
                if not is_duplicate(file_name[file_idx], idx_row):
                    add_error_log_and_data(file_name[file_idx], row_data_err_dupl_fcst, data_err_fcst, 'Issue Exception', idx_row)
                if not validate(idx_row):
                    add_error_log_and_data(file_name[file_idx], row_data_err_vali_fcst, data_err_fcst,'Working/NonWorking Exception',idx_row)
                if not is_ssm(idx_row):
                    add_error_log_and_data(file_name[file_idx], row_data_err_ssmc_fcst, data_err_fcst, 'SMM Exception', idx_row)
                if not validate_kpi(idx_row):
                    add_error_log_and_data(file_name[file_idx], row_data_err_kpi_fcst, data_err_fcst, 'KPI Exception', idx_row)
                # 符合所有标准的数据格式化后写入Summary文件
                if is_null(idx_row) and calc_total_budget(idx_row) and is_duplicate(file_name[file_idx], idx_row) and validate(idx_row) and is_ssm(idx_row) and validate_kpi(idx_row):
                    for idx_col in range(len(header_std)):
                        to_replace(idx_col, idx_row)
