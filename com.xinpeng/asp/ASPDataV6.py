...
def calc_total_budget(row_idx):
    ...
    # print(temp)
    # print(total_budget)
    # print(df_fcst[header_std[32]][row_idx])
    # print(is_eq)
    # print('==================')
    return is_eq
...
def to_replace_null(col_idx, row_idx):
    ...
    if col_idx in [11, 12, 13, 14, 17, 2]:
        if str(df_fcst[header_std[col_idx]][row_idx]) in ["0", "nan"]:
...
def to_split_fcstact(row_idx, f_name):
    ...
    if str(df_fcst[header_std[0]][row_idx]) == issue_name and str(df_fcst[header_std[17]][row_idx]) in ['nan', '0' ,'' ,' ']:
        ...
        df_fcst[header_std[0]][row_idx] = 'Actual'
    return flag
...
file_path_b = "D:/ASP - Erin/FCST/Error/"
file_path_c = "D:/ASP - Erin/FCST/Error/Files"
file_path_d = "D:/ASP - Erin/Actual/Data&Log/"
file_path_e = "D:/ASP - Erin/Actual/Error/"
file_path_f = "D:/ASP - Erin/Actual/Error/Files"
mkdir(file_path_a)
mkdir(file_path_b)
mkdir(file_path_c)
# 几个生成的excel路径及文件名
file_new = "D:/ASP - Erin/Data&Log/Summary.xlsx"
file_err = "D:/ASP - Erin/Data&Log/Error.xlsx"
file_header_err = "D:/ASP - Erin/Error/Error_header.xlsx"
file_MKT = "D:/ASP - Erin/Data&Log/MKT.xlsx"
file_MKT_FCST = "D:/ASP - Erin/Data&Log/MKT_FCST.xlsx"
file_CC = "D:/ASP - Erin/Data&Log/APAC CC.xlsx"
file_CC_FCST = "D:/ASP - Erin/Data&Log/APAC CC_FCST.xlsx"
# 创建几个excel的Writer引擎
...
writer_CC = pd.ExcelWriter(path=file_CC, mode='w', engine='xlsxwriter')
writer_CC_FCST = pd.ExcelWriter(path=file_CC_FCST, mode='w', engine='xlsxwriter')
...
for file_idx in range(len(file_name)):
    ...
    if not header_result.all():
        ...
        for idx_row in range(len(df_fcst[header_std[0]])):
            ...
            row_data_for_CC = []
            row_data_for_CC_fcst = []
            row_data_for_MKT = []
            row_data_for_MKT_fcst = []
            # 判断idx_row的前两列是否为空
            ...
            # 符合所有标准的数据格式化后写入Summary文件
            elif to_dept('APAC CC', idx_row) and to_split_fcstact(idx_row, file_name[file_idx]):
                for idx_col in range(len(header_std)):
                    ...
                    row_data_for_CC.append(df_fcst[header_std[idx_col]][idx_row])
                data_CC.append(row_data_for_CC)
            elif to_dept('APAC CC', idx_row) and  not to_split_fcstact(idx_row, file_name[file_idx]):
                for idx_col in range(len(header_std)):
                    to_replace(idx_col, idx_row)
                    to_simplify(idx_row)
                    to_replace_null(idx_col, idx_row)
                    ...
                    to_upper(idx_row, 6, 'NonWorking')
                    to_float(idx_col, idx_row)
                    row_data_for_CC_fcst.append(df_fcst[header_std[idx_col]][idx_row])
                data_CC_fcst.append(row_data_for_CC_fcst)
            elif to_dept('MKT', idx_row) and to_split_fcstact(idx_row, file_name[file_idx]):
                for idx_col in range(len(header_std)):
                    to_replace(idx_col, idx_row)
                    to_simplify(idx_row)
                    to_replace_null(idx_col, idx_row)
                    ...
                    to_upper(idx_row, 6, 'NonWorking')
                    to_float(idx_col, idx_row)
                    row_data_for_MKT.append(df_fcst[header_std[idx_col]][idx_row])
                data_MKT.append(row_data_for_MKT)
            elif to_dept('MKT', idx_row) and not to_split_fcstact(idx_row, file_name[file_idx]):
                for idx_col in range(len(header_std)):
                    to_replace(idx_col, idx_row)
                    to_simplify(idx_row)
                    to_replace_null(idx_col, idx_row)
                    ...
                    to_upper(idx_row, 6, 'NonWorking')
                    to_float(idx_col, idx_row)
                    row_data_for_MKT_fcst.append(df_fcst[header_std[idx_col]][idx_row])
                data_MKT_fcst.append(row_data_for_MKT_fcst)
            else:
                for idx_col in range(len(header_std)):
                    to_replace(idx_col, idx_row)
                    to_simplify(idx_row)
...
frame_MKT = pd.DataFrame(data_MKT, columns=header_std)
frame_MKT.to_excel(excel_writer=writer_MKT, index=False, header=True)
to_format(writer_MKT, frame_MKT, data_MKT, [], [15, 16, 19, 32], [17, 18])

frame_MKT_fcst = pd.DataFrame(data_MKT_fcst, columns=header_std)
frame_MKT_fcst.to_excel(excel_writer=writer_MKT_FCST, index=False, header=True)
to_format(writer_MKT_FCST, frame_MKT_fcst, data_MKT_fcst, [], [15, 16, 19, 32], [17, 18])

frame_CC = pd.DataFrame(data_CC, columns=header_std)
frame_CC.to_excel(excel_writer=writer_CC, index=False, header=True)
to_format(writer_CC, frame_CC, data_CC, [], [15, 16, 19, 32], [17, 18])

frame_CC_fcst = pd.DataFrame(data_CC_fcst, columns=header_std)
frame_CC_fcst.to_excel(excel_writer=writer_CC_FCST, index=False, header=True)
to_format(writer_CC_FCST, frame_CC_fcst, data_CC_fcst, [], [15, 16, 19, 32], [17, 18])
