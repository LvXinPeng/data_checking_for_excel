...
col_old = ['经销商代码', 'LEVEL', '一级技师', '二级技师', '三级电气技师', '三级机械技师', '三级空调技师', '四级技师', '一级专属服务顾问', '二级专属服务顾问', '保修专员', '高级保修专员',
       '客户服务经理', '混合动力技师', '经销商内训师', '一级钣金技师', '二级钣金技师', '三级钣金技师', '服务管家', '一级配件专员', '二级配件专员', '客户关系经理',
       '车间管理']
col_new = ['经销商代码', 'LEVEL', '一级技师', '二级技师', '三级电气技师', '三级机械技师', '三级空调技师', '四级技师', '一级专属服务顾问', '二级专属服务顾问', '保修专员',
    ...
    if get_months(input_year, str(df_dealer.ix[d_idx, 5])) >= 12:
                ...
                if 0 < num_avg <= 100:
                    ...
                    for o_idx in range(len(df_stdo['月均入厂台次'])):
                        if df_stdo['月均入厂台次'][o_idx] == '0 - 100':
                            for oc_idx in range(len(col_old) - 2):
                                row_data_old.append(df_stdo[col_old[oc_idx + 2]][o_idx])
        ...
        for n_idx in range(len(df_stdn['建店规模'])):
            if df_dealer['Format'][d_idx] == df_stdn['建店规模'][n_idx]:
                ...
                for nc_idx in range(len(col_new) - 2):
                    row_data_new.append(df_stdn[col_new[nc_idx + 2]][n_idx])
            elif df_dealer['Format'][d_idx] == '<V150' and df_stdn['建店规模'][n_idx] == 'V100':
                row_data_new.append(df_dealer['Format'][d_idx])
                for nc_idx in range(len(col_new) - 2):
                    row_data_new.append(df_stdn[col_new[nc_idx + 2]][0])
            elif df_dealer['Format'][d_idx] == 'V700' and df_stdn['建店规模'][n_idx] == 'V700 – V900':
                row_data_new.append(df_dealer['Format'][d_idx])
                for nc_idx in range(len(col_new) - 2):
                    row_data_new.append(df_stdn[col_new[nc_idx + 2]][4])
                ...
                row_data_new.append(df_dealer['Format'][d_idx])
                for nc_idx in range(len(col_new) - 2):
                    row_data_new.append(df_stdn[col_new[nc_idx + 2]][5])
