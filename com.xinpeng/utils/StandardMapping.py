...
input_year = input("Please input date like '2019-08-20':\n")
    ...
    if get_months(input_year, str(df_dealer.ix[d_idx, 5])) >= 12:
        ...
        for a_idx in range(len(df_num_avg['经销商代码'])):
            ...
            if df_dealer['Dlr Code'][d_idx] == df_num_avg['经销商代码'][a_idx]:
                ...
                row_data_old.append(df_num_avg['经销商代码'][a_idx])
                num_sum = 0
        ...
        for a_idx in range(len(df_num_avg['经销商代码'])):
            if df_dealer['Dlr Code'][d_idx] == df_num_avg['经销商代码'][a_idx]:
                # 内外层code相同，则添加code到row_data数组里
                row_data_new.append(df_num_avg['经销商代码'][a_idx])
                num_sum = 0
                for idx in range(len(df_num_avg.columns) - 3):
                    num_sum = num_sum + int(df_num_avg.ix[a_idx, df_num_avg.columns[idx + 3]])
                num_avg = num_sum / 12
                if 0 < num_avg <= 100:
                    row_data_new.append('V1')
                    for n_idx in range(len(df_stdn['建店规模'])):
                        if df_stdn['建店规模'][n_idx] == 'V100':
                            for nc_idx in range(len(df_stdn.columns) - 1):
                                row_data_new.append(df_stdn.ix[n_idx, df_stdn.columns[nc_idx + 1]])
                elif 101 <= num_avg <= 150:
                    row_data_new.append('V2')
                    for n_idx in range(len(df_stdn['建店规模'])):
                        if df_stdn['建店规模'][n_idx] == 'V150':
                            for nc_idx in range(len(df_stdn.columns) - 1):
                                row_data_new.append(df_stdn.ix[n_idx, df_stdn.columns[nc_idx + 1]])
                elif 151 <= num_avg <= 300:
                    row_data_new.append('V3')
                    for n_idx in range(len(df_stdn['建店规模'])):
                        if df_stdn['建店规模'][n_idx] == 'V300':
                            for nc_idx in range(len(df_stdn.columns) - 1):
                                row_data_new.append(df_stdn.ix[n_idx, df_stdn.columns[nc_idx + 1]])
                elif 301 <= num_avg <= 600:
                    row_data_new.append('V4')
                    for n_idx in range(len(df_stdn['建店规模'])):
                        if df_stdn['建店规模'][n_idx] == 'V600':
                            for nc_idx in range(len(df_stdn.columns) - 1):
                                row_data_new.append(df_stdn.ix[n_idx, df_stdn.columns[nc_idx + 1]])
                elif 601 <= num_avg <= 900:
                    row_data_new.append('V5')
                    for n_idx in range(len(df_stdn['建店规模'])):
                        if df_stdn['建店规模'][n_idx] == 'V700 – V900':
                            for nc_idx in range(len(df_stdn.columns) - 1):
                                row_data_new.append(df_stdn.ix[n_idx, df_stdn.columns[nc_idx + 1]])
                elif 901 <= num_avg <= 1800:
                    row_data_new.append('V6')
                    for n_idx in range(len(df_stdn['建店规模'])):
                        if df_stdn['建店规模'][n_idx] == 'V1200 – V1800':
                            for nc_idx in range(len(df_stdn.columns) - 1):
                                row_data_new.append(df_stdn.ix[n_idx, df_stdn.columns[nc_idx + 1]])
...
# print(data_old)
# print(data_new)
col = ['经销商代码', 'LEVEL', '一级技师', '二级技师', '三级电气技师', '三级机械技师', '三级空调技师', '四级技师',
       '混合动力技师', '一级钣金技师', '二级钣金技师', '三级钣金技师', '一级专属服务顾问', '二级专属服务顾问', '服务管家',
       '客户关系经理', '客户服务经理', '经销商内训师', '一级配件专员', '二级配件专员', '保修专员', '高级保修专员',
       '车间管理']
frame_old = pd.DataFrame(data_old, columns=col)
frame_old.to_excel(writer_old, index=False, header=True)
writer_old.save()
writer_old.close()
frame_new = pd.DataFrame(data_new)
frame_new.to_excel(writer_new, index=True, header=True)
writer_new.save()
writer_new.close()
print('Finished')
