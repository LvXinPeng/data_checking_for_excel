try:
    ...
    def is_life_cycle(df, header, row_idx):
        ...
        for lifecycle_item in std:
            if str(df[header[4]][row_idx]).replace(' ', '').upper() == str(lifecycle_item).replace(' ', '').upper():
                is_life = True
                df[header[4]][row_idx] = lifecycle_item
        return is_life
    ...
    def is_carline(df, header, row_idx):
        ...
        for carline_item in std:
            if str(df[header[3]][row_idx]).replace(' ', '').upper() == str(carline_item).replace(' ', '').upper():
                is_car = True
                df[header[3]][row_idx] = carline_item
        return is_car
    ...
    def validate_team(df, header, row_idx):
        validation = False
        for dept_item in read_dept(dept_std):
            if str(df[header[1]][row_idx]).replace(' ', '').upper() == dept_item.replace(' ', '').upper():
                df[header[1]][row_idx] = dept_item
                for team_item in read_sub_std(dept_std, df[header[1]][row_idx]):
                    if str(df[header[2]][row_idx]).replace(' ', '').upper() == team_item.replace(' ', '').upper():
                        df[header[2]][row_idx] = team_item
                        validation = True
        return validation
    ...
    def validate(df, header, row_idx):
        ...
        std_nonworking = read_std(nonworking_std)  # ['N.A.']
        if str(df[header[6]][row_idx]).replace(' ', '').upper() == 'NONWORKING':
            df[header[6]][row_idx] = 'NonWorking'
            for sale_item in std_nonworking:
                if str(df[header[7]][row_idx]).replace(' ', '').upper() == sale_item.replace(' ', '').upper():
                    df[header[7]][row_idx] = sale_item
                    validation_sale = True
                    for cate_item in  read_sub_std(nonworking_std, 'Category'):
                        if str(df[header[8]][row_idx]).replace(' ', '').upper() == cate_item.replace(' ', '').upper():
                            validation_cate = True
                            df[header[8]][row_idx] = cate_item
                            for cate_type_item in read_sub_std(nonworking_std, cate_item):
                                if str(df[header[9]][row_idx]).replace(' ', '').upper() == cate_type_item.replace(' ', '').upper():
                                    df[header[9]][row_idx] = cate_type_item
                                    validation_acti = True
        elif str(df[header[6]][row_idx]).replace(' ', '').upper() == 'WORKING':
            df[header[6]][row_idx] = 'Working'
            for sale_item in std_working:
                if str(df[header[7]][row_idx]).replace(' ', '').upper() == sale_item.replace(' ', '').upper():
                    df[header[7]][row_idx] = sale_item
                    validation_sale = True
                    for cate_item in read_sub_std(working_std, 'Category'):
                        if str(df[header[8]][row_idx]).replace(' ', '').upper() == cate_item.replace(' ', '').upper():
                            validation_cate = True
                            df[header[8]][row_idx] = cate_item
                            for cate_type_item in read_sub_std(working_std, cate_item):
                                if str(df[header[9]][row_idx]).replace(' ', '').upper() == cate_type_item.replace(' ', '').upper():
                                    df[header[9]][row_idx] = cate_type_item
                                    validation_acti = True
    ...
    def add_summary(file_name, header, data):
        if len(file_name) > 0:
            df_summary = pd.read_excel(file_name[0], dtype=str)
            for row_idx in range(len(df_summary[header[0]])):
                ...
                data.append(r_data)
            os.chmod(file_name[0], stat.S_IWRITE)
    ...
    path_fcst = folder_path + "Result/Checking/FCST/"
    path_actual = folder_path + "Result/Checking/Actual/"
    path_bgt = folder_path + "Result/Checking/Budget/"
    ...
    # # 创建存放excel的文件夹
    file_path_a = folder_path + "Result/Checking/FCST/"
    file_path_b = folder_path + "Result/Checking/Header Error/"
    file_path_c = folder_path + "Result/Checking/Header Error/Files/"
    file_path_d = folder_path + "Result/Checking/Actual/"
    file_path_e = folder_path + "Result/Checking/Budget/"
    # mkdir(file_path_a)
    # mkdir(file_path_b)
    # mkdir(file_path_c)
    # mkdir(file_path_d)
    # 几个生成的excel路径及文件名
    file_new_actual = folder_path + "Result/Checking/Actual/Act_Summary.xlsx"
    file_new_fcst = folder_path + "Result/Checking/FCST/FCST_Summary.xlsx"
    file_new_bgt = folder_path + "Result/Checking/Budget/Budget_Summary.xlsx"
    file_err_actual = folder_path + "Result/Checking/Actual/Act_Error.xlsx"
    file_err_fcst = folder_path + "Result/Checking/FCST/FCST_Error.xlsx"
    file_err_bgt = folder_path + "Result/Checking/Budget/Budget_Error.xlsx"
    ...
    add_summary(file_readable_name_fcst, header_std_plus, data_new_fcst)
    add_summary(file_readable_name_actual, header_std_plus, data_new_actual)
    add_summary(file_readable_name_bgt, header_std_plus, data_new_bgt)
    show_time(start)
    # 从文件列表遍历读取文件
    for file_idx in range(len(file_writeable_name)):
        ...
        if not header_result:
            ...
            try:
                ...
                # shutil.move(file_writeable_name[file_idx], file_path_b)
                # print(e)
                # print("Please return to the parent folder, modify its header and copy it to this folder to execute the program again.")
                pass
        else:
    ...
    file_new_bgt = folder_path + "Result/Checking/Budget/Budget_Summary.xlsx"
    # 判断文件是否存在
    if os.path.exists(file_new_actual):
        os.chmod(file_new_actual, stat.S_IREAD)
    if os.path.exists(file_new_fcst):
        os.chmod(file_new_fcst, stat.S_IREAD)
    if os.path.exists(file_new_bgt):
        os.chmod(file_new_bgt, stat.S_IREAD)
