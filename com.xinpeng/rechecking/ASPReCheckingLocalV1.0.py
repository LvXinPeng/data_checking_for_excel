...
# 创建几个excel的Writer引擎
re_writer_new = pd.ExcelWriter(path=file_new, mode='w', engine='xlsxwriter')
re_writer_new_fcst = pd.ExcelWriter(path=file_new_fcst, mode='w', engine='xlsxwriter')
re_writer_err = pd.ExcelWriter(path=file_err, mode='w', engine='xlsxwriter')
re_writer_err_fcst = pd.ExcelWriter(path=file_err_fcst, mode='w', engine='xlsxwriter')
re_writer_err_header = pd.ExcelWriter(path=file_header_err, mode='w', engine='xlsxwriter')
# 取Issue值
...
for file_idx in range(len(file_name)):
    ...
    if not header_result:
        ...
        try:
            ...
            # print(file_name[file_idx])
            os.remove(file_name[file_idx])
            # print(file_name)
    else:
        # 符合header标准，遍历文件的所有行，验证行数据
        for idx_row in range(len(df_fcst[header_std[0]])):
            if to_split_fcstact(idx_row):
                ...
                if not is_null(idx_row):
                    add_error_log_and_data(file_name[file_idx], row_data_err_null, data_err, 'Issue/Dept Null Exception', idx_row)
                ...
                if not validate_kpi(idx_row):
                    add_error_log_and_data(file_name[file_idx], row_data_err_kpi_fcst, data_err_fcst, 'KPI Exception',
                                           idx_row)
                # 符合所有标准的数据格式化后写入Summary文件
                if is_null(idx_row) and calc_total_budget(idx_row) and is_duplicate(issue_val, idx_row) and validate(
...
frame_header_err.to_excel(re_writer_err_header, index=False, header=True)
to_format(re_writer_err_header, frame_header_err, data_err_header, [0, 1], [], [])
# 将Actual异常数据写入Error excel
frame_err = pd.DataFrame(data_err, columns=header_err)
frame_err.to_excel(re_writer_err, index=False, header=True)
to_format(re_writer_err, frame_err, data_err, [0, 1, 2], [18, 19, 22, 35], [20, 21])
# 将FCST异常数据写入Error excel
frame_err_fcst = pd.DataFrame(data_err_fcst, columns=header_err)
frame_err_fcst.to_excel(re_writer_err_fcst, index=False, header=True)
to_format(re_writer_err_fcst, frame_err_fcst, data_err_fcst, [0, 1, 2], [18, 19, 22, 35], [20, 21])
# 将Actual标准数据写入Error excel
frame_new = pd.DataFrame(data_new, columns=header_std)
frame_new.to_excel(re_writer_new, index=False, header=True)
to_format(re_writer_new, frame_new, data_new, [], [15, 16, 19, 32], [17, 18])
# 将FCST标准数据写入Error excel
frame_new_fcst = pd.DataFrame(data_new_fcst, columns=header_std)
frame_new_fcst.to_excel(re_writer_new_fcst, index=False, header=True)
to_format(re_writer_new_fcst, frame_new_fcst, data_new_fcst, [], [15, 16, 19, 32], [17, 18])
print('================')
print(data_new)
print(data_new_fcst)
print(data_err)
